...
$(distdir): FORCE
	...
	cp src/*.y $(distdir)/src
	cp src/*.l $(distdir)/src
...
check: all
	cd tests && $(MAKE) $@
	export LD_LIBRARY_PATH=lib/; tests/$(test_exec) tests/tests.serc

memtest: all
	cd tests && $(MAKE) $@
	export LD_LIBRARY_PATH=lib/; valgrind --tool=memcheck --leak-check=yes  --leak-check=full --track-origins=yes -v tests/$(test_exec) tests/tests.serc
